...
2. Copy the orchestration rule — plugins can't ship rules, so this one step is manual:
   ...
   ```bash
   curl -o ~/.claude/rules/fable-orchestrator.md https://raw.githubusercontent.com/pithpusher/claude-code-fable-orchestrator/main/rules/agents.md
   ```
...
4. Turn on the **Orchestrator** output style. It puts the orchestrator contract in the system prompt, where it outranks rules files. Pick it in `/config` under **Output style**, or set it in `~/.claude/settings.json`:
...
- **An advisor closes at least half the gap** to the stronger model in the measured pairings where the session kept consulting. A low-effort session can stop consulting and then score below the session model alone, which happened to Sonnet 5 on SWE-bench Pro. Anthropic didn't report Sonnet + advisor beating Opus alone.
- **It loses when consulted on most tasks.** On one benchmark the Opus + Fable pairing matched Fable alone at medium effort, within noise, at about 2.6 times the cost, because the advisor fired on nearly every task.
...
Rules are advice, and a strong orchestrator will argue its way past advice — repeated test runs showed Fable building the deliverable itself whenever the task felt like "analysis." So the plugin ships a `PreToolUse` hook that makes the contract mechanical for the **main session only** (subagents are never restricted — their own tool grants constrain them):
...
The denial message tells the orchestrator what to do instead. It is a pattern match on the command text, not a sandbox. Known gaps: a script that lives in the scratchpad but writes into the project, in-place edits wrapped in `find -exec` or `xargs`, wrappers prefixed with `env` or `nohup`, and deletions (`rm`). The output style tells the orchestrator that a denial is the signal to brief the builder, not an obstacle to route around.
...
| *session on Fable* | Fable 5.1 | small edits only (hook limits) | Lean orchestrator: write specs, dispatch, verify, integrate. | Read large code, bulk refactor, write deliverables |
...
### Default: Opus session with a Fable advisor

```mermaid
flowchart TD
    S[Session · Opus] -.->|after orientation, before done| A[Advisor · Fable]
    A -.->|guidance| S
    S -->|locate, past ~5 searches| Sc[scout · Haiku]
    S -->|doc or web facts| Re[researcher · Sonnet]
    S -->|whole feature| B[builder · Opus]
    S -->|bulk mechanical edits| BS[builder · Sonnet]
    B --> V{Session runs verify}
    BS --> V
    V -->|fail: re-brief once| B
    V -->|risk trigger| R[refuter · Opus]
    V -.->|stuck, cause unknown| D[debugger · Opus]
```

The session reads and edits directly, so small edits never leave it. It consults the advisor about twice per task and delegates only work that would take more than a handful of tool calls. Verify and refuter rules are the same as in the Fable mode below.

### Fable-conducted: lean orchestrator

```mermaid
flowchart TD
    F[Session · Fable] -->|locate| Sc[scout · Haiku]
    F -->|facts| Re[researcher · Sonnet]
    F -->|every deliverable, one brief per feature| B[builder · Opus]
    B -->|report + artifact path| V{Fable runs verify}
    V -->|fail: re-brief once| B
    V -->|risk trigger| R[refuter · Opus]
    V -.->|stuck, cause unknown| D[debugger · Opus]
```
...
3. The session reruns the verify command itself in one call and reads only the failures and the summary line. On failure it re-briefs the same builder once with the output, then intervenes.
...
Four mechanisms, written into the agent definitions and the output style rather than left to hope:
...
- **Verify output, not diffs.** The orchestrator runs the verify command once and reads only the failures and the summary line. It doesn't read the diff.
...
| `CLAUDE_CODE_SUBAGENT_MODEL` | `env` block in `settings.json` | Default for any subagent that has no `model:` and isn't given one, such as general-purpose and other plugins' agents. It doesn't change the built-in `Explore` and `Plan` agents; add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1` to force it onto every subagent. |
...
| `availableModels` | `settings.json` | Hard allowlist. A blocked family alias such as `opus` falls back to the newest allowed version of that family; any other blocked value runs on the session model. Also trims your `/model` picker. |
...
No. It adds five agents alongside them. `CLAUDE_CODE_SUBAGENT_MODEL` alone doesn't change their model: `Explore` inherits the session model capped at Opus, and `Plan` inherits it uncapped. On Fable the guard hook denies both. Elsewhere, define your own agent named `Explore` with `model: haiku` to override the built-in, or set `CLAUDE_CODE_DISABLE_EXPLORE_PLAN_AGENTS=1` to remove both.
...
Those route *per request* with a learned policy. This routes *per role* with a fixed pin, and puts the cost/quality bet in one place (builder + refuter both Opus). Claude Code has no per-request router. The closest runtime piece is the advisor, which escalates to a stronger model at decision points rather than routing each request.

**Why not ship the rule inside the plugin?**
Plugins can install agents, skills, commands, hooks, MCP servers, and output styles. They don't install rules files, so `rules/agents.md` is a one-time copy.
...
- [fable5-opus5-orchestrator](https://github.com/Rylaa/fable5-opus5-orchestrator): Fable stays the session model while Sonnet 5 does the work, with a requirements ledger, enforced guard hooks, and automatic teammate reaping.
- [claude-code-workflow-orchestration](https://github.com/barkain/claude-code-workflow-orchestration): automatic task decomposition, parallel agent execution, and plan-mode integration.
